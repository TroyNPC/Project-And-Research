...
class BoruvkasMST:
    ...
    def boruvka_mst(self):
        ...
        while num_components > 1:
            cheapest = [-1] * self.V  # Store the cheapest edge for each component
            edges_added = False  # Flag to check if any edge is added this iteration
            ...
            # Go through all components and add the cheapest edge
            for i in range(self.V):
                if cheapest[i] != -1:  # If there is a cheapest edge for this component
                    ...
                    if set_u != set_v:  # Ensure they are in different sets
                        self.union(set_u, set_v)
                        self.mst_edges.append((u, v, w))
                        self.total_cost += w
                        print(f"{u + 1} - {v + 1}  ||  {w}")
                        num_components -= 1
                        edges_added = True  # Mark that an edge has been added

            # If no edges were added in this iteration, break to avoid infinite loop
            if not edges_added:
    ...
    def visualize_full_graph(self):
        ...
        print(f"Full graph saved as '{output_path}'.")
    ...
    def visualize_mst(self):
        ...
        for u, v, w in self.edges:
            ...
            if (u, v, w) in self.mst_edges or (v, u, w) in self.mst_edges:
                ...
                dot.edge(str(u + 1), str(v + 1), label=str(w), color='red')  # Non-MST edges

        output_path = os.path.join(self.output_folder, 'user_boruvka_mst.png')
        dot.render(filename=output_path, format='png', cleanup=True)
        print(f"MST visualization saved as '{output_path}'.")
        os.startfile(output_path)
