...
void display_state(const State &state, const string &message) {
    cout << message << endl;
    for (int i = 0; i < Nb; ++i) {
        for (int j = 0; j < Nb; ++j) {
...
public:
    int input() {
        ...
        display_state(state, "After AddRoundKey:");
        ...
        for (int round = 1; round <= 10; ++round) {
            ...
            if (encrypt) {
                sub_bytes(state);
                display_state(state, "After SubBytes (Round " + to_string(round) + "):");
                cout << "\n";

                shift_rows(state);
                display_state(state, "After ShiftRows (Round " + to_string(round) + "):");
                cout << "\n";

                if (round < 10) {
                    mixColumns(state);
                    display_state(state, "After MixColumns (Round " + to_string(round) + "):");
                    cout << "\n";
                }
            } else {
                inv_shift_rows(state);
                display_state(state, "After InvShiftRows (Round " + to_string(round) + "):");
                cout << "\n";

                inv_sub_bytes(state);
                display_state(state, "After InvSubBytes (Round " + to_string(round) + "):");
                cout << "\n";

                if (round < 10) {
                    inv_mix_columns(state);
                    display_state(state, "After InvMixColumns (Round " + to_string(round) + "):");
                    cout << "\n";
                }
            }

            add_round_key(state, round_key);
            display_state(state, "After AddRoundKey (Round " + to_string(round) + "):");
            cout << "\n\n";
        }
